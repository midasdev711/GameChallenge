<template>
  <div class="d-flex match-record" :style="cardStyle">
    ...
    <div class="magic-block d-flex">
      <Avatar
        imageUrl="character2.png"
        :size="54"
        type="rounded"
        borderColor="#1C1F33"
        :badge="record.champLevel"
      />
      <SummonerGroup :summonerUrls="summonerUrls" />
    </div>
    <div class="match-record-detail">
      <p class="mb-0 font-size1 font-grey font-weight-bold">
        <span class="mr-1">{{ record.kills }}</span
        >/<span class="ml-1 mr-1">{{ record.deaths }}</span
        >/<span class="ml-1">{{ record.assists }}</span>
      </p>
      <p class="mb-0 font-color2 font-size2">{{ record.kda }} KDA</p>
    </div>
    <div class="match-record-detail2">
      <p class="mb-2 font-grey font-size1 font-weight-bold">
        {{ record.cs }} CS
      </p>
      <p class="pt-2 mb-0 font-color2 font-size2">
